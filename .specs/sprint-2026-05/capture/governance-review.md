...
- [ ] **Spec and implementation contract are outdated (requires update)**: The debugging changes (`--frame N`, rotation animation, minimum 2-frame driver quirk, `capture_cube_scene` 5-parameter signature, `glReadBuffer(GL_BACK)`, `glClearColor`) are not reflected in `.specs/sprint-2026-05/capture/spec.md` or `.specs/sprint-2026-05/capture/implementation-contract.md`. These documents must be updated to match the current implementation. See "Spec/contract update recommendations" above.
...
1. **Update `.specs/sprint-2026-05/capture/spec.md`** to reflect the debugging changes (`--frame N`, rotation, min 2 frames, updated function signatures, updated usage text, removed/modified non-goal).
2. **Update `.specs/sprint-2026-05/capture/implementation-contract.md`** to match the updated spec and current implementation.
