...
- Define a `coordination.md` file per feature at `.specs/{{SPRINT}}/<feature>/coordination.md`.
...
- The orchestrator creates `.specs/{{SPRINT}}/<feature>/coordination.md` at the start of the workflow.
...
**Then** the orchestrator reads `.specs/{{SPRINT}}/<feature>/coordination.md` ONLY (not the full spec-critic.md or spec.md files)
...
**Then** the sub-agent reads `.specs/{{SPRINT}}/<feature>/coordination.md`
...
|---|---|---|
| AC-001 | coordination.md is created by the orchestrator at `.specs/{{SPRINT}}/<feature>/coordination.md` at the start of the workflow. | Inspect file existence after orchestrator begins a new feature. |
...
1. **Clarification** — Normal clarification with the human. After clarification, the orchestrator:
   - Creates `.specs/{{SPRINT}}/<feature>/` directory (if not exists).
   - Creates `.specs/{{SPRINT}}/<feature>/coordination.md` using `docs/templates/coordination-template.md`.
...
2. **Read coordination.md** from `.specs/{{SPRINT}}/<feature>/coordination.md`.
...
- All agents have read/write access to coordination.md (same as other files in `.specs/{{SPRINT}}/<feature>/`).
...
- All existing sub-agents use file read/write tools that can target arbitrary paths under `.specs/{{SPRINT}}/<feature>/`.
