...
- [ ] **Implementation contract is stale**
  The existing `.specs/sprint-2026-06/ci-docker-publish/implementation-contract.md` was rejected during loop #2. It still uses `docker/build-push-action` for the CI pull step and states "No `docker/login-action` needed." With the spec now accepted, the implementation contract must be rewritten to match the corrected spec.
