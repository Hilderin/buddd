...
- [x] **BL-001: Asset-manager spec non-goals not updated** — Fixed: updated `.specs/sprint-2026-06/asset-manager/spec.md` non-goals and out-of-scope references to reflect that glTF model loading is now supported via `ModelAsset`.
...
- **`.specs/sprint-2026-06/asset-manager/spec.md`**: Update non-goals (line 62) and out-of-scope (line 1113) to remove glTF as a non-goal, reflecting that `ModelAsset` is now supported. **(Required for BL-001)**
- **`.specs/sprint-2026-06/gltf-model-loading/spec.md`**: Update Appendix C tinygltf Git tag from `v2.10.0` to `v2.9.7` to match ADR-018 and actual implementation. Update `add_model_to_world()` signature to show non-const `ModelNode&`.
