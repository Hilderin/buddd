...
- [ ] **IMPL-007 (CLI Command Evolution, `docs/specs/cli-command-evolution/implementation-contract.md`)** specified adding new CLI tests to `tests/version_test.cpp`. The tests were instead extracted to separate `tests/cmd_test.cpp` and `tests/demo_test.cpp` files, with helper code moved to `tests/test_helpers.h`. This deviates from the contract.

- [ ] **IMPL-009 (3D Cube Demo, `docs/specs/3d-cube-demo/implementation-contract.md`)** specified manually adding `model_tests.cpp` to `tests/CMakeLists.txt` in both `if(BUDDD_HAS_DISPLAY)` and `else()` branches. The actual implementation replaced the entire CMakeLists.txt with a GLOB_RECURSE approach — a fundamentally different change.

- [ ] **SPEC-007 (CLI Command Evolution, `docs/specs/cli-command-evolution/spec.md`)** non-goal: "No changes to the test infrastructure directory structure (`tests/`)." The creation of `test_helpers.h`, `cmd_test.cpp`, and `demo_test.cpp` changes the test infrastructure structure.
