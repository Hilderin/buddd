...
| File | Role |
|---|---|
| `type_registry.h` / `type_registry.cpp` | `TypeRegistry` — static class mapping each C++ type to YAML encode/decode, string conversion, and validation callbacks. Nine built-in types pre-registered at startup (`float`, `int32_t`, `bool`, `std::string`, `Vec3`, `Vec4`, `Quat`, `Color`, `std::shared_ptr<Model>`). External code registers custom types via `TypeRegistry::register_type<T>(TypeInfo<T>)`. **F-06 (component properties)**: Extended `TypeEntry` struct with `yaml_encode_any`/`yaml_decode_any` dispatch functions (`std::function`). Added type-erased `yaml_encode(type_index, any, ctx) -> Result<YAML::Node>` and `yaml_decode(type_index, node, ctx) -> Result<std::any>` static methods. `register_type<T>()` populates the dispatch functions automatically. |
...
| `component_info.h` | `ComponentInfoBase` (type-erased base) and `ComponentInfo<T>` (typed template). Each component type has one info object holding its canonical string name, factory function, and property descriptors. `ComponentInfo<T>` provides three `add_property<PropType>()` overloads: (A) convention-based (deferred for v1), (B) simple lambdas, (C) context-aware lambdas. **New**: `add_property()` now computes a default checker lambda and passes it to the `Property` constructor. `serialize()` skips null-node properties (properties whose value matches the registered default), producing compact YAML output. **F-06 (component properties)**: Added `property_serialize(comp, idx, ctx) -> YAML::Node` and `property_deserialize(comp, idx, node, ctx) -> Result<void>` pure virtual methods on `ComponentInfoBase`, implemented in `ComponentInfo<T>` delegating to the Property's `serialize()`/`deserialize()` methods. A parallel `raw_getters_` vector stores direct-value lambdas to bypass the default-skipping behavior of `Property::serialize()` for editor use. |
...
| `editor_dialog.h` | `Dialog` (abstract), `CustomDialog` (concrete), `DialogButton` (struct) | Header-only dialog abstraction. `Dialog` provides pure virtual `id()`, `title()`, `draw_content()`, and non-virtual `request_close()`, `should_close()`, `handle_escape()`. `CustomDialog` takes a content function, button list, and optional `on_close` callback. `DialogButton` stores `label`, `label_id`, `callback`, and optional `shortcut`. Buttons are rendered by the framework after `draw_content()` with auto-close after any click. See [Editor Dialog Abstraction spec](/.specs/sprint-2026-06/editor-dialog-abstraction/spec.md). |
...
| File | Role |
|---|---|
| `inspector_editors.h` | `InspectorTypeEditor` abstract base class, `TypedInspectorEditor<T>` typed template, `EditorFlags` struct, `InspectorTypeEditorRegistry` static class, `register_builtin_inspector_editors()` declaration. **F-06 (component properties)**: Added virtual `draw_any(label, any&, type_index, flags, ctx) -> bool` to `InspectorTypeEditor`, type-extracting `draw_any()` override in `TypedInspectorEditor<T>`, and static `draw_any()` dispatch method on `InspectorTypeEditorRegistry`. See [F-05 spec](/.specs/sprint-2026-06/inspector-transform/spec.md) and [F-06 component properties spec](/.specs/sprint-2026-06/inspector-component-properties/spec.md). |
| `inspector_editors.cpp` | 9 built-in editor implementations (float, int, bool, string, Vec2, Vec3, Vec4, Quat, Color), fallback text-input helpers, fallback read-only display, `register_builtin_inspector_editors()` registration. **F-06 (component properties)**: `draw_fallback_readonly()` gains ImGui context guard. See [F-05 spec](/.specs/sprint-2026-06/inspector-transform/spec.md). |
...
| File | Role |
|---|---|
| `editor.h` | `Editor` class — central orchestrator. Lifecycle: `Editor()` → `setup(ctx)` → `update(ctx) x N` → `draw_ui(ctx) x N` → `shutdown()`. Public methods: `setup()` (registers menus/panels/shortcuts, sets `IniFilename`), `update()` (processes shortcuts via `ShortcutRegistry`, delegates to `menu->update()`/`panel->update()`), `add_menu()`/`add_panel()` (registration), `draw_ui()` (now 7-phase rendering), `shutdown()`, `world()` (returns `World&`). **Editor Dialog Abstraction**: Added `open_dialog(std::unique_ptr<Dialog>) -> bool` public method for opening reusable modal dialogs with ID-based dedup. Private state: `std::vector<std::unique_ptr<Dialog>> dialogs_` (owned dialog instances), `std::unordered_set<std::string> opened_dialog_ids_` (removed — OpenPopup is now called unconditionally each frame, making the tracking set unnecessary). Phase 4 of `draw_ui()` is the general dialog rendering phase — iterates `dialogs_`, opens popups via `ImGui::OpenPopup((dialog->title() + "###" + dialog->id()).c_str())` (the `"title###id"` pattern prevents ImGui ID collisions), renders via `ImGui::BeginPopupModal`, dispatches Escape to the topmost dialog only, and removes closed dialogs via `std::erase_if`. The About popup, save-prompt, error modals, and delete confirmation all use the Dialog abstraction — no ad-hoc ImGui popup code remains. `DialogButton::callback` type changed from `void()` to `bool()` (return `true` to close dialog). New convenience helpers: `open_message_dialog()`, `open_error_dialog()`, `open_confirm_dialog()`, `open_ok_cancel_dialog()`. New `defer()` mechanism enqueues actions into `deferred_actions_` (flushed at start of `draw_ui()`). **F-01 additions**: Public scene management methods: `mark_dirty()`, `clear_dirty()`, `is_dirty()`, `current_file_path()`, `new_scene()`, `open_scene(path)`, `save_scene()`, `save_scene_as(path)`. Private state: `bool dirty_`, `std::optional<std::string> current_file_path_`, `PendingOp pending_op_`. Private methods: `update_window_title()`, `build_title_string()`, `draw_pending_op_modal()` (simplified — opens a `CustomDialog` via `open_dialog()`), `execute_pending_op()`. **Removed state/methods**: `pending_file_path_`, `error_modal_title_`, `error_modal_message_`, `show_error_modal_`, `SavePromptResult` enum, `save_prompt_requested_`, `save_prompt_seen_`, `draw_save_prompt_modal()`, `show_error_modal()`, `draw_error_modals()`. **F-01 (SDL3 native dialogs)**: Removed `show_file_dialog_`, `file_dialog_action_`, `draw_file_dialog()`, `show_save_prompt_modal_`, `save_prompt_result_`, `handle_dirty_before_op()`. Added `request_exit_next_frame_` flag for Quit→Save As callback. No `#include <ImGuiFileDialog.h>` — dialog operations go through `Platform::show_open_file_dialog()` / `show_save_file_dialog()`. Enums: `SavePromptResult` (`Save`, `Discard`, `Cancel`), `PendingOp` (`None`, `NewScene`, `OpenScene`, `Quit`). |
