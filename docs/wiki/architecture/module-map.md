...
| `render_device.cpp` | Factory implementation: dispatches to OpenGL or Headless backend based on `native_handle()` value. Passes `Window&` to both backend constructors. Diagnostic output (backend selection errors) uses `BUDDD_LOG_ERROR` via tag `Render`. |
