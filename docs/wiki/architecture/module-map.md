...
| `entity.h` | `Entity` class — 16-byte lightweight handle (`World*` + `EntityId`). Inline template methods for component operations. **New**: `name()` / `set_name()` accessors for entity naming (default empty string). |
| `entity.cpp` | Entity non-inline method implementations — all delegate to `World`. `Entity::name()` delegates to `World::get_name()`, `Entity::set_name()` delegates to `World::set_name()`. |
| `world.h` | `World` class — top-level container managing entity lifecycle, tree hierarchy, deferred destruction. Template methods for component dispatch (`add_component`, `get_component`, `remove_component`) and type-based iteration (`each<T>()`) defined inline. Camera registration API (`register_camera`, `unregister_camera`, `active_camera`) stores a `CameraComponent&` reference in an `std::optional<CameraComponent&>` member. **New**: `add_component_raw(EntityId, unique_ptr<Component>)` for runtime-type component injection (used by SceneLoader). **New**: `entity_count() -> size_t` introspection method. EntityNode gains `std::string name_` field. |
| `world.cpp` | World implementation including internal `EntityNode` type, slot-based storage, `flush_destroyed()` logic, and `mark_for_destroy()` iterative traversal. Implements `get_name()`, `set_name()`, `add_component_raw()`, and `entity_count()`. |
