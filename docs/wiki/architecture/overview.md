...
| `buddd_editor` | Static library | `src/editor/` | Editor library: Editor class, EditorApp, editor infrastructure, scene management (New/Open/Save/Save As/Quit), dirty state, OS file dialogs via Platform abstraction (SDL3 native). Links buddd_engine. |
...
- `./build/debug/src/cmd/buddd edit` — opens the editor (1280×800 window by default, or the saved size/position from a previous session; title "Untitled — Buddd Editor") with a main menu bar (File > New Scene/Ctrl+N, Open Scene/Ctrl+O, Save Scene/Ctrl+S, Save Scene As/Ctrl+Shift+S, Quit/Ctrl+Q; Edit > Undo/Ctrl+Z, Redo/Ctrl+Shift+Z/Ctrl+Y; Help > About), five dockable placeholder panels (Scene, Properties, Console, Project, Assets), dirty state tracking (`*` in window title when modified), OS file dialogs (SDL3 native via Platform abstraction) for Open/Save As, all modal popups (save-prompt, error modals, delete confirmation) using the reusable Dialog abstraction, OS close-button interception with save-prompt, and docking layout persistence via `buddd_editor.ini`.
...
- **F-01: Editor scene management**: The Editor now supports File > New Scene (Ctrl+N), Open Scene (Ctrl+O), Save Scene (Ctrl+S), Save Scene As (Ctrl+Shift+S), and Quit (Ctrl+Q) with dirty state tracking (`*` in window title), OS file dialogs (SDL3 native via Platform abstraction), save-prompt modals, and error modals for load/save failures.
...
- **F-01: Platform file dialog abstractions**: Platform interface now provides `show_open_file_dialog()` and `show_save_file_dialog()` pure virtual methods with a `FileDialogCallback` type alias. PlatformSDL3 implements SDL3 native dialogs (`SDL_ShowOpenFileDialog` / `SDL_ShowSaveFileDialog`) with heap-allocated callback pattern (no mutex/queue). PlatformHeadless implements no-op dialogs (immediate callback with `std::nullopt`). ImGuiFileDialog was removed from the build system.
...
- **F-01**: Editor scene management: `Editor::new_scene()` (replaces World with fresh empty World), `Editor::open_scene(path)` (loads YAML via SceneLoader, restores World on failure), `Editor::save_scene()` (saves via SceneSaver, no-op if clean with path), `Editor::save_scene_as(path)` (saves to new path, updates file path). Dirty state: `Editor::mark_dirty()`, `Editor::clear_dirty()`, `Editor::is_dirty()`. Window title: `Editor::build_title_string()` returns formatted title with dirty `*` suffix. `Editor::update_window_title()` calls `Window::set_title()`. Save-prompt via `CustomDialog` with button callbacks (Save/Don't Save/Cancel), driven by `PendingOp` enum — `SavePromptResult` and `draw_save_prompt_modal()` removed. Error modals via `open_error_dialog()` convenience helper. Delete confirmation via `CustomDialog` + `defer()`. All popups use the Dialog abstraction. Platform dialog integration via direct `Platform::show_open_file_dialog()` / `Platform::show_save_file_dialog()` calls (no `draw_file_dialog()` — ImGuiFileDialog removed). OS close interception via `Platform::set_on_close_request()`.
