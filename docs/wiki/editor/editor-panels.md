...
> **F-05 + F-06 + F-07 implementation**: The `InspectorTypeEditorRegistry` (in `src/editor/inspector_editors.h/.cpp`) provides reusable ImGui editor widgets for the 9 built-in types listed below, with a fallback text-input path for unregistered types (uses `TypeRegistry::to_string()`/`from_string()`). The Transform section (Position, Rotation, Scale) uses these editors via `InspectorTypeEditorRegistry::draw<Vec3>()` and `InspectorTypeEditorRegistry::draw<Quat>()`. As of F-06 (UX Polish), Vec2/Vec3/Vec4/Quat editors use composite axis input widgets (colored drag-handle + InputFloat) and no longer render a property-name label — the `id` parameter is used only for ImGui PushID scoping. As of F-06 (Component Properties), component sections use `InspectorTypeEditorRegistry::draw_any()` for runtime type-erased dispatch. As of F-07 (Undo Polish), `draw_axis_widget()` was flipped to place the InputFloat on the left and the colored drag handle on the right; the standalone float editor was replaced with a composite InputFloat + gray drag handle; and float/int/bool/string editors use hidden `"##val"` labels.
> 
> > **SPEC-NNNN (Inspector Indefinite Drag)**: The drag handles in `draw_axis_widget()` and the float editor lambda now use relative mouse mode instead of `ImGui::GetMouseDragDelta()`. On handle activation (`IsItemActivated`), the cursor is hidden and relative mouse mode is enabled via `Window::set_mouse_capture(true)`, and the cursor's starting position is saved. During the drag (`IsItemActive`), value changes are computed from `InputSystem::mouse_delta().x` (raw per-frame delta accumulated since drag start), enabling indefinite drag past window boundaries. On release (`IsItemDeactivated`), relative mouse mode is disabled and the cursor is warped back to the drag start position via `InputSystem::set_mouse_position()`. This affects all float, Vec2, Vec3, Vec4, and Quat drag handles. The int editor (uses `ImGui::DragInt` natively), bool, string, and Color editors are unchanged. See [SPEC-NNNN spec](/.specs/sprint-2026-06/inspector-indefinite-drag/spec.md).
...
|---|---|---|
| `bool` | Checkbox (hidden `"##val"` label as of F-07) |
| `int` | `ImGui::DragInt` (hidden `"##val"` label as of F-07) |
| `float` | Composite widget: `ImGui::InputFloat` (`"%.5g"` format) on the left + gray drag handle on the right (hidden `"##val"` label) |
| `std::string` | `ImGui::InputText` (hidden `"##val"` label as of F-07) |
| `Vec2` | Composite axis widgets: InputFloat on left, colored drag-handle (red=X, green=Y) on right |
| `Vec3` | Composite axis widgets: InputFloat on left, colored drag-handle (red=X, green=Y, blue=Z) on right |
| `Vec4` | Composite axis widgets: InputFloat on left, colored drag-handle (red=X, green=Y, blue=Z, gray=W) on right |
...
| `Quat` | Euler angles in degrees via composite axis widgets: InputFloat on left, colored drag-handle (Pitch=red, Yaw=green, Roll=blue) on right, wrapped to [-180, 180] |
...
- **F-07 additions**: New `SetTransformCommand` class in `src/editor/commands/set_transform_command.h` — stores all 3 transform properties (Position, Rotation, Scale) as native `Vec3`/`Quat`, with `execute()`/`undo()` writing all three values and calling `mark_dirty()`. `Command::try_update_new_value(YAML::Node const&, EditorContext const&, std::string_view property_name) -> bool` virtual method added to `src/editor/command.h` (base returns false). `CommandStack::peek_undo() -> Command*` added to `src/editor/command_stack.h/.cpp`. `SetComponentPropertyCommand::try_update_new_value()` override with entity + property identity checks added. `draw_axis_widget()` layout flipped: InputFloat on left, colored drag handle on right. Float editor replaced with composite widget (`ImGui::InputFloat` + gray drag handle on right, `"%.5g"` format). Float/int/bool/string editors use hidden `"##val"` labels (no duplicate labels in component property tables). `draw_transform_section()` now snapshots old values before edit and pushes/merges `SetTransformCommand` with per-property undo granularity. `draw_component_sections()` uses `peek_undo()` + `try_update_new_value()` merge logic with `execute()` after merge. See [F-07 spec](/.specs/sprint-2026-06/properties-panel-undo-polish/spec.md).
...
- [SPEC-F-07 — Properties Panel Undo Polish](/.specs/sprint-2026-06/properties-panel-undo-polish/spec.md) — SetTransformCommand, undo merging, peek_undo/try_update_new_value, float composite widget, axis handle right-side, hidden labels
